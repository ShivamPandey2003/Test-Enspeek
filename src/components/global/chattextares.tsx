...
import { LuListChecks, LuMessageCircle, LuSendHorizontal } from "react-icons/lu";
...
const ChatTextArea: React.FC<ChatTextAreaProps> = ({
  ...
  const promptItems = React.useMemo(
    () => buildPromptItems(setDraftMessage),
    [setDraftMessage],
  );
  ...
  return (
    <>
      ...
      <div
        className={cn(
          "z-50 flex @container cursor-text flex-col rounded-3xl border border-slate-100 bg-white p-4 shadow-[0_4px_24px_rgba(79,70,229,0.08)] transition-all duration-300 ease-in-out",
          isChatOpen
      ...
      >
        {/* {visibleSuggestions.length > 0 && (
          ...
          >
            <div className="-m-1 flex @lg:flex-row @max-lg:flex-col @lg:items-center gap-2 p-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              <div>
